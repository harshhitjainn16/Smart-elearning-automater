...
notesBtn.addEventListener('click', () => {
  ...
  notesBtn.textContent = isHidden ? '📓 Take Note (Ctrl+Shift+N)' : '📓 Hide Notes';
...
saveNoteBtn.addEventListener('click', async () => {
  ...
  // Parse tags
  const tagsText = noteTagsInput.value.trim();
...
exportNotesBtn.addEventListener('click', async () => {
  chrome.runtime.sendMessage({
    ...
    if (response && response.success) {
      ...
      const videoGroups = {};
      notes.forEach(note => {
        if (!videoGroups[note.videoUrl]) {
          videoGroups[note.videoUrl] = [];
        }
        videoGroups[note.videoUrl].push(note);
      });
      
      for (const [url, videoNotes] of Object.entries(videoGroups)) {
        const firstNote = videoNotes[0];
        markdown += `## ${firstNote.videoTitle}\n`;
        markdown += `**Platform:** ${firstNote.platform}\n`;
        markdown += `**URL:** ${url}\n\n`;
        
        videoNotes.forEach(note => {
          markdown += `### [${note.formattedTime}] ${note.noteText}\n`;
      ...
      a.href = url;
      a.download = `video-notes-${Date.now()}.md`;
      a.click();
...
async function openNoteModal() {
  // Get current timestamp from active tab
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs[0]) {
      chrome.tabs.sendMessage(tabs[0].id, {
        ...
        if (chrome.runtime.lastError || !response || !response.success) {
          // No video found or not on supported page
          currentNoteData = {
            timestamp: 0,
            formattedTime: '00:00',
            videoTitle: 'General Note',
            videoUrl: tabs[0].url,
            platform: 'other'
          };
        } else {
          currentNoteData = response;
        }
        
        // Update modal
        noteTimestamp.textContent = currentNoteData.formattedTime;
        noteTextInput.value = '';
        noteTagsInput.value = '';
...
function displayNotes(notes) {
  if (notes.length === 0) {
    notesList.innerHTML = '<p style="text-align: center; opacity: 0.7;">No notes yet. Click "Add Note" or press Ctrl+Shift+N while watching!</p>';
    return;
  }
  
  // Show latest 10 notes
    ...
    return `
      <div class="note-item">
        <div class="note-header">
          <span class="note-timestamp" data-url="${note.videoUrl}" data-timestamp="${note.timestamp}" title="Click to jump to this moment">
            ⏱️ ${note.formattedTime}
          </span>
          <div class="note-actions">
            <button class="note-action-btn delete-note" data-url="${note.videoUrl}" data-id="${note.id}">🗑️</button>
          </div>
        </div>
        <div class="note-video-title">📹 ${note.videoTitle}</div>
        <div class="note-text">${escapeHtml(note.noteText)}</div>
        ${note.tags.length > 0 ? `
          <div class="note-tags">
        ...
        ` : ''}
        <div class="note-meta">
          <span>${date} • ${note.platform}</span>
        </div>
      </div>
    `;
  }).join('');
  
  // Add event listeners for timestamp jumps
  document.querySelectorAll('.note-timestamp').forEach(element => {
    element.addEventListener('click', (e) => {
      ...
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (tabs[0] && tabs[0].url === url) {
          // Same video, jump to timestamp
          chrome.tabs.sendMessage(tabs[0].id, {
            action: 'jumpToTimestamp',
            timestamp: timestamp
          }, (response) => {
            if (response && response.success) {
              console.log('Jumped to timestamp:', timestamp);
            }
          });
        } else {
          // Different video, open URL with timestamp
          if (url.includes('youtube.com')) {
            window.open(`${url}&t=${timestamp}s`, '_blank');
          } else {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'noteAdded' || request.action === 'noteDeleted') {
